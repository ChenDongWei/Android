...
public class AntiVirusDao {
    /**
     * @param version 服务器的病毒库版本
     * @return 病毒库是否一致
     */
    public static boolean isNewVirus(int version) {
        boolean res = false;
        SQLiteDatabase database = SQLiteDatabase.openDatabase("/data/data/com.thxy" +
                ".mobileguard/files/antivirus.db", null, SQLiteDatabase.OPEN_READWRITE);
        Cursor cursor = database.rawQuery("select 1 from version where subcnt=?", new
                String[]{version + ""});

...
import com.cdw.googleplay.ui.activity.BaseActivity;
...
public class PagerTab extends ViewGroup {
		...
		if (viewTreeObserver != null) {//监听第一个的全局layout事件，来设置当前的mCurrentPosition，显示对应的tab
			viewTreeObserver.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
				...
				public void onGlobalLayout() {
					getViewTreeObserver().removeOnGlobalLayoutListener(this);//只需要监听一次，之后通过listener回调即可
	...
	private void addTextTab(final int position, String title) {
		...
		tab.setBackground(UIUtils.getDrawable(mTabBackgroundResId));
